...
Feature: E-commerce Order Pricing Promotions
  ...
  Scenario: Double Eleven bulk discount - 12 pieces of same product
    Given the Double Eleven bulk discount promotion is active:
      | minQuantity | discountRate |
      | 10          | 20           |
    When a customer places an order with:
      | productName | quantity | unitPrice |
      | 襪子          | 12       | 100       |
    Then the order summary should be:
      | originalAmount | bulkDiscount | totalAmount |
      | 1200           | 200          | 1000        |
    And the customer should receive:
      | productName | quantity |
      | 襪子          | 12       |

  Scenario: Double Eleven bulk discount - 27 pieces of same product
    Given the Double Eleven bulk discount promotion is active:
      | minQuantity | discountRate |
      | 10          | 20           |
    When a customer places an order with:
  ...
  Scenario: Double Eleven bulk discount - 10 different products no discount
    Given the Double Eleven bulk discount promotion is active:
      | minQuantity | discountRate |
      | 10          | 20           |
    When a customer places an order with:
